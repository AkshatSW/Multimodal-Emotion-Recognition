...
def audio_dash():
    ...
    return render_template('audio_dash.html', emo_other=major_emotion_other, prob_other=emotion_dist_other)
...
def text_1():
    ...
    df_words_co = df_words_co.groupby('WORDS').agg({'FREQ':'sum'}).reset_index()
    df_words_co.to_csv('static/js/db/words_common.txt', sep=',', index=False)
    
    session['text_info']["common_words"].append(df_words_co)
    
    df_words_perso = pd.read_csv('static/js/db/words_perso.txt', sep=',')
    df_words_perso.FREQ = df_words_perso.FREQ.apply(pd.to_numeric)
    df_words_perso = df_words_perso.groupby('WORDS').agg({'FREQ':'sum'}).reset_index()
    df_words_perso.to_csv('static/js/db/words_perso.txt', sep=',', index=False)
    
    session['text_info']["common_words"].append(df_words_perso)
    
    session['text_info']["num_words"].append(num_words)
    
    return render_template('text_1.html', trait=trait, probas=probas, data_traits=data_traits, probas_others=probas_others, trait_others=trait_others, common_words=common_words, num_words=num_words)
...
def text_pdf() :
    return render_template('text_pdf.html')

@app.route('/text_pdf_1', methods=['POST'])
def text_pdf_1() :
    if request.method == 'POST':
        f = request.files['file']
        file_path = os.path.join(tempdirectory, secure_filename(f.filename))
        f.save(file_path)
        text = extract_text_pdf(file_path)
        os.remove(file_path)
        text = preprocess_text(text)
        return render_template('text_1.html', text=text)
